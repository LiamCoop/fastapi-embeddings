...
// RetrievalHandler handles retrieval requests.
type RetrievalHandler struct {
	service *retrievalservice.Service
}

func NewRetrievalHandler(service *retrievalservice.Service) *RetrievalHandler {
...
func (h *RetrievalHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) != 4 || parts[0] != "v1" || parts[1] != "kb" || parts[3] != "retrieve" {
	...
	kbID := parts[2]
	var payload retrievalRequestJSON
...
type retrievalRequestJSON struct {
	...
	Filters      *retrievalFiltersJSON `json:"filters"`
}

type retrievalFiltersJSON struct {
...
func buildRetrievalRequest(kbID string, payload retrievalRequestJSON) (retrieval.Request, error) {
